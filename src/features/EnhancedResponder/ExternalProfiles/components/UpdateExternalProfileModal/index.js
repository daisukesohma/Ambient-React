...
const UpdateExternalProfileModal = () => {
  // const { account } = useParams()
  const dispatch = useDispatch()
  const darkMode = useSelector(state => state.settings.darkMode)
  const loading = useSelector(
    state => state.externalContacts.editContactLoading,
  )
  ...
  const onYesClick = () => {
    ...
    dispatch(updateContactRequested({ input: data }))
  }
...
UpdateExternalProfileModal.defaultProps = defaultProps
UpdateExternalProfileModal.propTypes = propTypes

export default UpdateExternalProfileModal
