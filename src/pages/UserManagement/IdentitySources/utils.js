...
export const trimIdentitySource = identitySource => {
  const {
    username,
    host,
    port,
    ...
    clientId,
    clientSecret,
  } = JSON.parse(identitySource.config)
  return {
    identitySourceId: identitySource.id,
    name: identitySource.name,
    host,
    type: makeSourceTypeName(identitySource.sourceType.name),
    username,
    lastSyncRequest: identitySource.lastSyncRequest,
    status: identitySource.lastSyncRequest
      ? identitySource.lastSyncRequest.status
      : null,
    port,
    dn,
    group,
    sourceTypeId: identitySource.sourceType.id,
    sourceType: identitySource.sourceType,
    tenantId,
    clientId,
    clientSecret,
    federationProfiles: identitySource.federationProfiles,
  }
}
