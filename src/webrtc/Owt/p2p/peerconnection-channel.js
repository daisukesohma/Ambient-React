...
class P2PPeerConnectionChannel extends EventDispatcher {
  ...
  constructor(config, localId, remoteId, signaling, isInitializer) {
    ...
    this._iceConnectionStateListeners = []
    this._onIceDisconnectedTimeout = null
  }
  ...
  getStats(mediaStream) {
    ...
    return Promise.reject(
      new ErrorModule.P2PError(ErrorModule.errors.P2P_CLIENT_INVALID_STATE),
    )
  }
  ...
  _SignalingMesssageHandler(message) {
    ...
    switch (message.type) {
      ...
      default:
        Logger.error(
          `Invalid signaling message received. Type: ${message.type}`,
        )
    }
  }
      ...
      this._publishingStreamTracks.forEach((mediaTrackIds, mediaStreamId) => {
        for (let i = 0; i < mediaTrackIds.length; i++) {
          if (mediaTrackIds[i] === id) {
            ...
            this._publishedStreamTracks
              .get(mediaStreamId)
              .push(mediaTrackIds[i])
          ...
          if (mediaTrackIds.length === 0) {
            if (!this._publishPromises.has(mediaStreamId)) {
              Logger.warning(
                `Cannot find the promise for publishing ${mediaStreamId}`,
              )
              continue
            }
  ...
  _streamInfoHandler(data) {
    ...
    this._remoteStreamInfo.set(data.id, {
      ...
      stream: null,
      mediaStream: null,
      receiver: null, // RTCRtpReceiver
      trackIds: data.tracks, // Track IDs may not match at sender and receiver sides. Keep it for legacy purposes.
    })
  }
  ...
  _onOffer(sdp) {
    Logger.debug(
      `About to set remote description. Signaling state: ${this._pc.signalingState}`,
    )
  ...
  _onAnswer(sdp) {
    ...
    this._pc
      ...
      .then(
        ...
        error => {
          Logger.debug(
            `Set remote description failed. Message: ${error.message}`,
          )
          this._stop(error, true)
        },
      )
  ...
  _onRemoteTrackAdded(event) {
    ...
    for (const stream of event.streams) {
      ...
      if (!this._remoteStreamInfo.get(stream.id).stream) {
        ...
        this._remoteStreamInfo.get(stream.id).receiver = event.receiver
      }
    }
    if (
  ...
  _onRemoteStreamAdded(event) {
    ...
    const audioTrackSource = this._remoteStreamInfo.get(event.stream.id).source
      .audio
    const videoTrackSource = this._remoteStreamInfo.get(event.stream.id).source
      .video
    ...
    const stream = new StreamModule.RemoteStream(
      undefined,
      this._remoteId,
      event.stream,
      sourceInfo,
  ...
  _onIceConnectionStateChange(event) {
    Logger.debug(
      `New ice connection state ${event.currentTarget.iceConnectionState}`,
    )
    if (event.currentTarget.iceConnectionState === 'closed') {
      if (this._onIceDisconnectedTimeout) {
        clearTimeout(this._onIceDisconnectedTimeout)
        this._onIceDisconnectedTimeout = null
      }
      ...
      event.currentTarget.iceConnectionState === 'connected' ||
      ...
      this._onIceDisconnectedTimeout = setTimeout(() => {
        this._stop(undefined, true)
      }, 5000)
    }
  }
    ...
    if (this._pc && this._pc.signalingState === 'stable') {
      ...
      for (let j = 0; j < this._pendingUnpublishStreams.length; j++) {
        ...
        this._unpublishPromises
          .get(this._pendingUnpublishStreams[j].mediaStream.id)
          .resolve()
  ...
  _drainPendingRemoteIceCandidates() {
    Logger.debug(`Draining ${this._remoteIceCandidates.length} candidates}.`)
  ...
  _createAndSendOffer() {
    ...
    this._pc
      ...
      .catch(e => {
        ...
        const error = new ErrorModule.P2PError(
          ErrorModule.errors.P2P_WEBRTC_SDP,
          e.message,
        )
        this._stop(error, true)
      })
  ...
  _logCurrentAndPendingLocalDescription() {
    if (this._pc && this._pc.currentLocalDescription) {
      Logger.info(
        `Current description: ${this._pc.currentLocalDescription.sdp}`,
      )
    }
    if (this._pc && this._pc.pendingLocalDescription) {
      Logger.info(
        `Pending description: ${this._pc.pendingLocalDescription.sdp}`,
      )
    }
  }
