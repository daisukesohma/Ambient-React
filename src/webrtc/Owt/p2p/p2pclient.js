...
const P2PClient = function(configuration, signalingChannel) {
  ...
  signaling.onMessage = function(data, origin) {
    Logger.debug(
      `Received signaling message from ${origin}: ${JSON.stringify(data)}`,
    )

    if (data.type === 'chat-closed') {
      if (channels.has(origin)) {
        parentThis.getOrCreateChannel(origin, false).onMessage(data)
   ...
   * @memberof Owt.P2P.P2PClient
   * @param {object} authObject A object that contains token or other auth for connecting to signaling server. The format of this token depends on signaling server's requirement.
  ...
  this.connect = function(authObject) {
    ...
    return new Promise((resolve, reject) => {
      signaling.connect(authObject).then(
        id => {
          myId = id
  ...
  this.disconnect = function() {
    Logger.debug('Disconnecting P2PClient.')
    if (state === ConnectionState.READY) {
      return
    }
  ...
  this.publish = function(remoteId, stream) {
    ...
    if (this.allowedRemoteIds.indexOf(remoteId) < 0) {
      return Promise.reject(
        new ErrorModule.P2PError(ErrorModule.errors.P2P_CLIENT_NOT_ALLOWED),
      )
    }
    return Promise.resolve(
      this.getOrCreateChannel(remoteId, true).publish(stream),
    )
  }
  ...
  this.send = function(remoteId, message) {
    ...
    return Promise.resolve(
      this.getOrCreateChannel(remoteId, true).send(message),
    )
  }
  ...
  this.restartIce = function(remoteId) {
    if (!channels.has(remoteId)) {
      Logger.warning(
        `No PeerConnection between current endpoint and ${remoteId}.`,
      )
    } else {
      channels.get(remoteId).restartIce()
    }
  }
  ...
  this.getOrCreateChannel = function(remoteId, isInitializer) {
    if (!channels.has(remoteId)) {
      ...
      pcc.addEventListener('ended', () => {
        Logger.debug(`Detected channel to ${remoteId} ended`)
        channels.delete(remoteId)
      })
      pcc.addEventListener('disconnected', event => {
        Logger.debug(`Detected channel to ${remoteId} disconnected`)
