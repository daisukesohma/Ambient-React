...
export class AudioTrackConstraints {
  ...
  constructor(source) {
    if (
      !Object.values(MediaFormatModule.AudioSourceInfo).some(v => v === source)
    ) {
...
export class VideoTrackConstraints {
  ...
  constructor(source) {
    if (
      !Object.values(MediaFormatModule.VideoSourceInfo).some(v => v === source)
    ) {
      throw new TypeError('Invalid source.')
    }
    /**
...
function isVideoConstrainsForScreenCast(constraints) {
  return (
    ...
    constraints.video.source === MediaFormatModule.VideoSourceInfo.SCREENCAST
  )
}
