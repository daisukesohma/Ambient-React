  ...
  if (mediaInfo.audio) {
    if (mediaInfo.audio.format) {
      audioCodec = new CodecModule.AudioCodecParameters(
        mediaInfo.audio.format.codec,
        mediaInfo.audio.format.channelNum,
    ...
    for (const videoInfo of mediaInfo.video.original) {
      if (videoInfo.format) {
        videoCodec = new CodecModule.VideoCodecParameters(
          videoInfo.format.codec,
          videoInfo.format.profile,
        )
      }
...
export function convertToSubscriptionCapabilities(mediaInfo) {
  ...
  if (mediaInfo.video) {
    ...
    if (
      mediaInfo.video &&
      mediaInfo.video.optional &&
      mediaInfo.video.optional.parameters
    ) {
