...
export class ConferencePeerConnectionChannel extends EventDispatcher {
  ...
  publish(stream, options, videoCodecs) {
    ...
    if (
      (this._isRtpEncodingParameters(options.audio) &&
    ...
    if (
      (options.audio === false || options.audio === null) &&
      (options.video === false || options.video === null)
    ) {
      return Promise.reject(
        new ConferenceError('Cannot publish a stream without audio and video.'),
      )
    }
    if (typeof options.audio === 'object') {
      ...
      for (const parameters of options.audio) {
        if (
          !parameters.codec ||
          typeof parameters.codec.name !== 'string' ||
          (parameters.maxBitrate !== undefined &&
            typeof parameters.maxBitrate !== 'number')
        ) {
          return Promise.reject(
    ...
    if (this._isOwtEncodingParameters(options.video)) {
      for (const parameters of options.video) {
        if (
          ...
          (parameters.maxBitrate !== undefined &&
            typeof parameters.maxBitrate !== 'number') ||
          (parameters.codec.profile !== undefined &&
    ...
    return new Promise((resolve, reject) => {
      this._publishPromise = { resolve, reject }
    })
  }
  ...
  subscribe(stream, options) {
      ...
      .then(data => {
        ...
        this._pc
          ...
          .then(
            desc => {
              ...
              this._pc.setLocalDescription(desc).then(
                ...
                function(errorMessage) {
                  Logger.error(
                    `Set local description failed. Message: ${JSON.stringify(
                      errorMessage,
                    )}`,
                  )
                },
            ...
            function(error) {
              Logger.error(
                `Create offer failed. Error info: ${JSON.stringify(error)}`,
              )
            },
          )
          .catch(e => {
    ...
    return new Promise((resolve, reject) => {
      this._subscribePromise = { resolve, reject }
    })
  }
  ...
  _muteOrUnmute(isMute, isPub, trackKind) {
    ...
    return this._signaling
      .sendSignalingMessage(eventName, {
        id: this._internalId,
        operation,
        data: trackKind,
      })
  ...
  _applyOptions(options) {
    ...
    videoOptions.bitrate = options.video.bitrateMultiplier
      ? `x${options.video.bitrateMultiplier.toString()}`
      : undefined
  ...
  _onIceConnectionStateChange(event) {
    ...
    Logger.debug(
      `ICE connection state changed to ${event.currentTarget.iceConnectionState}`,
    )
    if (
  ...
  _onStreamEvent(message) {
    ...
    if (this._publication && message.id === this._publication.id) {
      ...
      message.id === this._subscribedStream.id
    ) {
  ...
  _isRtpEncodingParameters(obj) {
    ...
    const param = obj[0]
    return (
      param.codecPayloadType ||
      param.dtx ||
      param.active ||
      param.ptime ||
      param.maxFramerate ||
      param.scaleResolutionDownBy ||
      param.rid
    )
  }
