 ...
 * @hideconstructor
 */
export class ConferenceClientConfiguration {
...
export const ConferenceClient = function(config, signalingImpl) {
  ...
  const signaling = signalingImpl || new Signaling()
  let me
  let room
  ...
  function onSignalingMessage(notification, data) {
    if (notification === 'soac' || notification === 'progress') {
      ...
      if (data.status === 'add') {
        ...
        if (
          data.data.field === 'audio.status' ||
          data.data.field === 'video.status'
        ) {
          channels.forEach(c => {
  ...
  function fireParticipantEvent(data) {
    if (data.action === 'join') {
      ...
      const event = new ParticipantEvent('participantjoined', { participant })
  ...
  function updateRemoteStream(streamInfo) {
    ...
    stream.extraCapabilities = StreamUtilsModule.convertToSubscriptionCapabilities(
      streamInfo.media,
    )
  ...
  function createRemoteStream(streamInfo) {
    ...
    const stream = new StreamModule.RemoteStream(
      streamInfo.id,
      streamInfo.info.owner,
      undefined,
      new StreamModule.StreamSourceInfo(audioSourceInfo, videoSourceInfo),
      streamInfo.info.attributes,
    )
    stream.settings = StreamUtilsModule.convertToPublicationSettings(
      streamInfo.media,
    )
  ...
  Object.defineProperty(this, 'info', {
    ...
    get: () => {
      ...
      return new ConferenceInfo(
        room.id,
        Array.from(participants, x => x[1]),
        Array.from(remoteStreams, x => x[1]),
        me,
      )
    },
  })
  ...
  this.join = function(tokenString) {
    return new Promise((resolve, reject) => {
      ...
      signaling.connect(host, isSecured, loginInfo).then(
        resp => {
  ...
  this.send = function(message, participantId) {
    ...
    return sendSignalingMessage('text', { to: participantId, message })
  }
