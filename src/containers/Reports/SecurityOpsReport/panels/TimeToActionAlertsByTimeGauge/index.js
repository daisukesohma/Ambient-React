...
const TimeToActionAlertsByTimeGauge = ({
  pollInterval,
  isTest,
  severities,
}) => {
  ...
  const onFilterChange = value => {
    ...
    if (filter.value !== value.value) {
      ...
      dispatch(
        changeAlertEvents({
          key: 'time_to_act',
          title: 'Alerts Acted on after Threshold',
          instances: filteredData,
        }),
      )
    }
  ...
  const title = 'Alerts Acted on Under'
  ...
  return (
    <BarGauge
      ...
      value={(() => {
        ...
        for (
          let i = 0;
          i < source.timeToActionLatencyDistribution.length;
          ++i
        ) {
          value += source.timeToActionLatencyDistribution[i].value
          if (source.timeToActionLatencyDistribution[i].name === filter.value) {
            return value
          }
        }
        return value
      ...
      total={(() => {
        for (
          ...
          if (source.timeToActionLatencyDistribution[i].name === filter.value) {
            return source.timeToActionLatencyDistribution[i].total
          }
        }
      ...
      title={title}
      description='All alerts that were acted on (comment/dispatch/resolve) '
      color='primary'
      moreComponent={
        <DropdownMenu
...
TimeToActionAlertsByTimeGauge.defaultProps = {
  pollInterval: null,
  isTest: false,
  severities: [],
}

TimeToActionAlertsByTimeGauge.propTypes = {
  pollInterval: PropTypes.number,
  isTest: PropTypes.bool,
  severities: PropTypes.array,
}

export default TimeToActionAlertsByTimeGauge
