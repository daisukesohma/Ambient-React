...
import { getCurrUnixTimestamp, msToUnix, unixToMs } from '../../../../utils'
...
const Timeline = ({ videoStreamKey, timezone }) => {
    ...
    dispatch(
      setVideoStreamValues({
        ...
        props: {
          ...
          clipWidth: 20,
          clipLeftPositionTS:
            Math.abs(newViewWindowPosition) + 60 + tsAtMidnight(0, timezone),
          clipRightPositionTS:
            Math.abs(newViewWindowPosition) +
            60 +
            tsAtMidnight(0, timezone) +
            20,
          exportStartTS:
            Math.abs(newViewWindowPosition) + 60 + tsAtMidnight(0, timezone),
          exportEndTS:
  ...
  const getTimestampOfMouse = useCallback(
    cursorPt => {
      ...
      return (
        ...
        tsAtMidnight(-subtractDays, timezone) +
        cursorPtX
      )
    },
    [isZoomIn, subtractDays, viewWindowPosition, timezone],
  )
  ...
  const renderTimelineMarkers = useMemo(() => {
    ...
    const nonMotionTs =
      subtractDays > 0 ? tsAtMidnight(-(subtractDays - 1), timezone) : nowTs
    if (
      ...
      let nonMotionX = 0
      let nonmotionWidth =
        subtractDays > 0 ? 86400 : nowTs - tsAtMidnight(0, timezone)
      if (subtractDays === 0) {
        // If it is current day
        nonmotionWidth = nowTs - tsAtMidnight(0, timezone)
      } else if (subtractDays === retention.nonmotionSegmentRetentionDays) {
        // Check to see if subtractDays is last day in retention
        nonMotionX = nowTs - tsAtMidnight(0, timezone)
      ...
      const nonmotionStartTs = tsAtMidnight(-subtractDays, timezone)
    ...
    if (catalogue) {
      ...
      const filtered = filterCatalogue(
        Math.abs(newViewWindowPosition) + tsAtMidnight(-subtractDays, timezone),
      )
      ...
      for (let i = 0; i < filtered.length; ++i) {
        ...
        if (subtractDays > 0) {
          currWidth = Math.min(
            ...
            msToUnix(
              tsAtMidnight(-(subtractDays - 1), timezone) - curr.startTs,
            ),
          )
    ...
    if (tsTimelineHighlight) {
      ...
      let highlightX = highlightTs - tsAtMidnight(-subtractDays, timezone)
      let maxTimeline
      let highlightWidth
      if (isZoomIn === 0) {
        highlightX /= 60
  ...
  const renderXAxisMarkers = useMemo(() => {
    ...
    for (let i = lowestMultiple; i <= largestMultiple; i += xAxisIncrements) {
      if (isZoomIn === 1) {
        unixTS = i + tsAtMidnight(0, timezone)
        isZoomOut = false
      } else {
        unixTS = i * SECONDS_IN_MINUTE + tsAtMidnight(0, timezone)
        isZoomOut = true
      }
      tsAtLocation = formatUnixTimeToReadable(
        unixTS,
        false,
        true,
        isZoomOut,
  ...
  }, [isZoomIn, viewWindowPosition, xAxisIncrements, timezone])
  ...
  const onTimelineHover = useCallback(
    event => {
      const cursorPt = getMouseCoordinatesWithinSvg(event)
      const unixTs = getTimestampOfMouse(cursorPt)
      const readableDate = formatUnixTimeToReadable(
        unixTs,
        false,
        true,
        false,
  ...
  const clipLeftPositionDisplay = useMemo(
    () =>
      clipLeftPositionTS
        ...
        : null,
    [clipLeftPositionTS, isZoomIn, timezone],
  )
  ...
  const clipRightPositionDisplay = useMemo(
    () =>
      clipRightPositionTS
        ...
        : null,
    [clipRightPositionTS, isZoomIn, timezone],
  )
  ...
  const readableTSPlaying = useMemo(() => {
    return isNaN(videoStreamTS) // eslint-disable-line
      ? formatUnixTimeToReadable(nowTs, false, true, false, timezone)
      : formatUnixTimeToReadable(videoStreamTS, false, true, false, timezone) // eslint-disable-line
  }, [videoStreamTS, nowTs, timezone])
